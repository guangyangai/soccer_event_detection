"""
Author: Guang Yang
Graph Snapshot data (defined with torch_geometric.data ) to be used in training
"""
...
class GraphSnapshotDataset(Dataset):
    ...
    def __getitem__(self, index):
        id = self.fold_x[index] #folder contains all the shots 
        snapshot_data = []
        # TODO: torch.nn.ModuleList()
        for snapshot_index in range(self.snapshot_num):
            data = np.load(
                "{}/{}_{}_{}.npz".format(self.data_path, id, snapshot_index, self.snapshot_num),  #graph data preprocessed and saved in npz
                allow_pickle=True
            )
